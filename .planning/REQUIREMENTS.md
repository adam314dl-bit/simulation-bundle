...
- [ ] **CTRL-01**: ParameterPanel auto-generates UI controls from a ParameterSchema supporting range (slider + numeric display), toggle, select, color, vec2 (two-handle), and group (collapsible) types
- [ ] **CTRL-02**: ParameterPanel supports reset all, compact mode, 1 or 2 column layout, real-time value display, and dark mode
